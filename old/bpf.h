...
Compilation options: -a faustMinimal.h -lang cpp -i -inpl -ct 1 -cn bpf -es 1 -mcd 16 -single -ftz 0
------------------------------------------------------------ */
...
class bpf : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a faustMinimal.h -lang cpp -i -inpl -ct 1 -cn bpf -es 1 -mcd 16 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow2 = std::max<float>(0.001f, float(fHslider1));
		float fSlow3 = 0.5f * (fSlow1 / fSlow2);
		float fSlow4 = fSlow3 + 1.0f;
		float fSlow5 = 0.5f * (fSlow1 / (fSlow2 * fSlow4));
		float fSlow6 = 1.0f / fSlow4;
		float fSlow7 = 0.0f - 2.0f * std::cos(fSlow0);
		float fSlow8 = 1.0f - fSlow3;
		float fSlow9 = 0.0f - fSlow5;
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			float fTemp0 = float(input0[i0]);
			fRec0[0] = fTemp0 - fSlow6 * (fSlow7 * fRec0[1] + fSlow8 * fRec0[2]);
			output0[i0] = FAUSTFLOAT(fSlow5 * fRec0[0] + fSlow9 * fRec0[2]);
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
		}
	}
